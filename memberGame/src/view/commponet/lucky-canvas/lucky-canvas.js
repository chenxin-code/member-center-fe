...
var computePadding = function(block) {
  var _a;
  var padding = ((_a = block.padding) === null || _a === void 0
      ? void 0
      : _a
          .replace(/px/g, "")
          .split(" ")
...
var Lucky = /** @class */ (function() {
  ...
  Lucky.prototype.initWindowFunction = function() {
    ...
    if (config.rAF) {
      ...
      this.rAF = function(callback) {
        return timeout_1(callback, 16.7);
      };
    } else {
  ...
  Lucky.prototype.zoomCanvas = function() {
    ...
    var canvasElement = config.canvasElement,
      dpr = config.dpr;
    var _b = [this.boxWidth * dpr, this.boxHeight * dpr],
      width = _b[0],
      height = _b[1];
  ...
  Lucky.prototype.drawImage = function(imgObj, xAxis, yAxis, width, height) {
    ...
    if (["WEB", "MP-WX"].includes(config.flag)) {
      ...
      ["UNI-H5", "UNI-MP", "TARO-H5", "TARO-MP"].includes(config.flag)
    ) {
      // 小程序中直接绘制一个路径
      drawImg = imgObj.path;
    }
...
var LuckyWheel = /** @class */ (function(_super) {
  ...
  function LuckyWheel(config, data) {
    ...
    _this.initComputed();
    // 创建前回调函数
    (_a = config.beforeCreate) === null || _a === void 0
      ? void 0
      : _a.call(_this);
    // 收集首次渲染的图片
    _this.init({
  ...
  LuckyWheel.prototype.handleClick = function(e) {
    ...
    (_a = this.startCallback) === null || _a === void 0
      ? void 0
      : _a.call(this, e);
  };
  /**
    ...
    return __awaiter(this, void 0, void 0, function() {
      ...
      return __generator(this, function(_a) {
        ...
        this.loadImg(imgInfo.src, imgInfo)
          ...
          .catch(function(err) {
            console.log(
              cellName + "[" + cellIndex + "].imgs[" + imgIndex + "] " + err
            );
          });
        return [2 /*return*/];
      });
  ...
  LuckyWheel.prototype.draw = function() {
    ...
    this.prizeRadius = this.blocks.reduce(function(radius, block, blockIndex) {
      ...
      block.imgs &&
        block.imgs.forEach(function(imgInfo, imgIndex) {
          if (!_this.blockImgs[blockIndex]) return;
          ...
          var _b = [
              ...
              _this.getHeight(imgInfo.top, radius * 2) - radius
            ],
            xAxis = _b[0],
            yAxis = _b[1];
          ctx.save();
      ...
      return (
        radius - _this.getLength(block.padding && block.padding.split(" ")[0])
      );
    }, this.Radius);
    // 计算起始弧度
    ...
    this.prizes.forEach(function(prize, prizeIndex) {
      ...
      prize.fonts &&
        prize.fonts.forEach(function(font) {
          ...
          if (
            Object.prototype.hasOwnProperty.call(font, "wordWrap")
              ? font.wordWrap
  ...
  LuckyWheel.prototype.run = function(num) {
    ...
    if (interval >= _defaultConfig.accelerationTime && prizeFlag !== void 0) {
      ...
      while (++i) {
        var endDeg =
          ...
          stopRange;
        var currSpeed =
          Tween[_defaultConfig.speedFunction].easeOut(
            this.FPS,
            this.stopDeg,
            endDeg,
    ...
    this.rotateDeg =
      (rotateDeg +
        Tween[_defaultConfig.speedFunction].easeIn(
          interval,
          0,
  ...
  LuckyWheel.prototype.getWidth = function(length, width) {
    ...
    if (isExpectType(length, "number")) return length;
    if (isExpectType(length, "string")) return this.changeUnits(length, width);
    return 0;
  };
  /**
   * 获取相对高度
  ...
  LuckyWheel.prototype.getHeight = function(length, height) {
    ...
    if (isExpectType(length, "number")) return length;
    if (isExpectType(length, "string")) return this.changeUnits(length, height);
    return 0;
  };
  /**
   * 获取相对(居中)X坐标
...
var LuckyGrid = /** @class */ (function(_super) {
  ...
  LuckyGrid.prototype.handleClick = function(e) {
    var _this = this;
    var ctx = this.ctx;
    __spreadArray(__spreadArray([], this.buttons), [this.button]).forEach(
      function(btn) {
        var _a;
        if (!btn) return;
        ...
        // 如果btn里有单独的回调方法, 优先触发
        if (typeof btn.callback === "function") btn.callback.call(_this, btn);
        // 最后触发公共回调
    ...
    return __awaiter(this, void 0, void 0, function() {
      ...
      return __generator(this, function(_a) {
        ...
        request = [
          this.loadImg(imgInfo.src, imgInfo),
          imgInfo["activeSrc"] &&
            this.loadImg(imgInfo["activeSrc"], imgInfo, "$activeResolve")
        ];
        Promise.all(request)
          .then(function(_a) {
            var defaultImg = _a[0],
              activeImg = _a[1];
            _this[imgName][cellIndex][imgIndex] = {
  ...
  LuckyGrid.prototype.draw = function() {
      ...
      function(_a, block) {
        ...
        if (hasBackground(background)) {
          drawRoundRect(
            ...
            h,
            r,
            _this.handleBackground(x, y, w, h, background)
          );
        }
        return {
          x: x + paddingLeft,
    ...
    // 计算单一奖品格子的宽度和高度
    this.cellWidth =
      (this.prizeArea.w - _defaultConfig.gutter * (this.cols - 1)) / this.cols;
    this.cellHeight =
      (this.prizeArea.h - _defaultConfig.gutter * (this.rows - 1)) / this.rows;
    // 绘制所有格子
    this.cells.forEach(function(cell, cellIndex) {
      var _a = _this.getGeometricProperty([cell.x, cell.y, cell.col, cell.row]),
        x = _a[0],
        y = _a[1],
        width = _a[2],
        height = _a[3];
      ...
      var background = isActive
        ...
        : cell.background || _defaultStyle.background;
      if (hasBackground(background)) {
        // 处理阴影 (暂时先用any, 这里后续要优化)
        var shadow = (isActive
          ? _activeStyle.shadow
          : cell.shadow || _defaultStyle.shadow
        )
        ...
        drawRoundRect(
          ...
          height,
          _this.getLength(
            cell.borderRadius ? cell.borderRadius : _defaultStyle.borderRadius
          ),
          _this.handleBackground(x, y, width, height, background)
        );
        // 清空阴影
      ...
      // 绘制图片
      cell.imgs &&
        cell.imgs.forEach(function(imgInfo, imgIndex) {
          if (!_this[cellName][cellIndex]) return;
          var cellImg = _this[cellName][cellIndex][imgIndex];
          if (!cellImg) return;
          var renderImg =
            (isActive && cellImg["activeImg"]) || cellImg.defaultImg;
          if (!renderImg) return;
          var _a = _this.computedWidthAndHeight(renderImg, imgInfo, cell),
            trueWidth = _a[0],
            trueHeight = _a[1];
          var _b = [
      ...
      // 绘制文字
      cell.fonts &&
        cell.fonts.forEach(function(font) {
          // 字体样式
          var style =
          ...
          lines.forEach(function(line, lineIndex) {
            ctx.fillText(
              line,
              x + _this.getOffsetX(ctx.measureText(line).width, cell.col),
              y +
                _this.getHeight(font.top, cell.row) +
                (lineIndex + 1) * _this.getLength(lineHeight)
            );
          });
        });
  ...
  LuckyGrid.prototype.getOffsetX = function(width, col) {
    ...
    return (
      (this.cellWidth * col + this._defaultConfig.gutter * (col - 1) - width) /
      2
    );
  };
  /**
