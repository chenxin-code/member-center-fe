      ...
      <a-row style="padding:20px;height:100%;">
        ...
        >
          ...
          <a-form-model-item label="区域" prop="areaId">
            <a-select v-model="form.areaId"  placeholder="请选择" @change="doInspectionPointId" style="width:350px;">
              <a-select-option
                :value="item.id"
          ...
          <a-form-model-item label="巡检点">
            <a-select v-model="form.inspectionPointId" placeholder="请选择"  style="width:350px;">
              <a-select-option
                :value="item.id"
                v-for="item in inspectionPointData"
                :key="item.id"
          ...
          <a-form-model-item label="扣罚分值">
			  <a-input style="width: 350px" type="number" v-model="form.deductScore" min=0
			   @blur="repair" @change="onInput"/>
		  </a-form-model-item>
...
export default {
  ...
  mounted() {
    this.getInRectificationRecord();
  },
  beforeRouteEnter(to, from, next) {
    const meta = to.meta;
    console.log(meta,from)
    for(let i = 0; i < meta.bread.length; i++){
      if(meta.bread[i].name == "巡查管理"){ // 点击面包屑后，吧id传回去
        meta.bread[i].path = `/qualityManagement/inspection`
        break;
      }
    }
    next();
  ...
  methods: {
   ...
   onInput(e){
    ...
    getInRectificationRecord() {
      ...
      console.log(param);
      api.getInRectificationRecord(param).then(res => {
        if (res.code === 200) {
          const {
            projectId,
            projectName,
          ...
          this.form.inspectionPointId = inspectionPointId || "";
    ...
    doInspectionPointId(){
      this.getInspectionPointList(() => {
        this.form.inspectionPointId = this.inspectionPointData.length ? this.inspectionPointData[0].name : undefined
      });
    },
    //获取巡检点列表
    getInspectionPointList(callback) {
        const para = {
          pageIndex: 1,
          pageSize: 10000,
          projectId: this.projectId,
        ...
        api.getInspectionPointList(para).then(res => {
          console.log(res);
          this.inspectionPointData = res.data.datalist;
          callback && callback()
        });
    },
    onSubmit() {
