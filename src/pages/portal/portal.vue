  ...
  <div id="portal" slot="content">
    <div class="portal-header">
      <div class="time-logo-content">
        <img class="time-logo" :src="require('@/assets/img/portal/logo.png')" />
      </div>
          ...
          <template v-for="(menuItem, index) in menu_block.children">
            <template v-if="menuItem.type === 'other'">
              <div
                class="menu-item"
                :key="index"
                @click="turn_other(menuItem.authKeys[0])"
                v-if="hasRangeAuthorityWithoutProject(menuItem.authKeys)"
              >
                <img class="menu-item-icon" :src="menuItem.icon" alt="icon" />
                <span class="menu-item-title">
                  {{ menuItem.title }}
                </span>
                <img class="menu-item-next" :src="require('@/assets/img/portal/icon_right.png')" alt="icon right" />
              </div>
            </template>
            <template v-else>
              <div
                class="menu-item"
                :key="index"
                @click="turn_route(menuItem.route)"
                v-if="hasRangeAuthorityWithoutProject(menuItem.authKeys)"
              >
                <img class="menu-item-icon" :src="menuItem.icon" alt="icon" />
                <span class="menu-item-title">
...
export default {
      ...
      api.getPermission().then(res => {
        if (res.code === 200) {
          ...
          PORTAL_MENUS.map(menuBlock => {
            const filteredMenuBlock = menuBlock.children.filter(x => hasRangeAuthorityWithoutProject(x.authKeys));
  ...
  watch: {
    portalMenusHas: {
      handler(newVal) {
        console.log('portalMenusHas newVal :>> ', newVal);
      },
      immediate: true,
      deep: true
    }
