<template>
  <div id="neighborhoodLife">
    <div class="content-header">邻里生活管理</div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      onAdd() {
        debounce(()=>{
          this.$router.push({path: "/advertise/neighborhoodLifeSet"})
        })
      },
      //编辑
      onEdit(operation) {
      ...
      getAdvertiseList() {
        ...
        const { projectId, title, type, status, updateTime   } = this.$refs.form.getFieldsValue();
        const para = {
          advertAreaIds: [5],
          projectId,
