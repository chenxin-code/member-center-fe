<template>
  <div id="advertiseArea">
    <div class="content-header">广告区域管理</div>
      ...
      <a-row style="padding:20px;height:100%;">
        <a-col flex:auto>
          ...
          >
            ...
            <template slot="putPort" slot-scope="operation">
              <div class="editable-row-operations">
                <span v-if="operation.launch === 0">AppC</span>
                <span v-if="operation.launch === 1">小程序</span>
                <span v-if="operation.launch === 2">AppC 小程序</span>
              </div>
            </template>
            <template slot="validTime" slot-scope="operation">
              <div class="editable-row-operations">
                <span>{{moment(operation.startValidTime).format("YYYY-MM-DD HH:mm") + " ~ " + moment(operation.endValidTime).format("YYYY-MM-DD HH:mm")}}</span>
              </div>
            </template>
...
<script>
  import api from "@/api";
  import { ADVERTISE_SKIP_TYPE, LOCATION} from "@/constance";
  ...
  export default {
      ...
      return {
        ...
        tableColumns: [
          ...
          },
          {
            ...
            width: 120,
            ellipsis: true,
            scopedSlots: { customRender: "putPort" },
          },
          {
            title: "位置",
            key: "location",
            dataIndex: "advertAreaId",
            width: 120,
            ellipsis: true,
            customRender: (val) => LOCATION[val] || val
          },
          {
            title: "优先级",
    ...
    mounted () {
      this.$refs.form.setFieldsValue({
        ...
        putPort: this.formList[4].selectOptions[0]?.id,
        location: this.formList[5].selectOptions[0]?.id,
      });
      setTimeout(()=>{
        this.scrollY = this.$refs.content_main.offsetHeight - 345 + "px";
      },100)
    },
    beforeRouteLeave(to, from, next) {
      if (to.name === "advertiseAreaSet") {
    ...
    methods:{
      ...
      onAdd() {
        debounce(()=>{
          this.$router.push({path: "/advertise/advertiseAreaSet"})
        })
      },
      //编辑
      onEdit(operation) {
      ...
      getAdvertiseList() {
        ...
        const { projectId, title, type, status, putPort, location, validTime, updateTime   } = this.$refs.form.getFieldsValue();
        const para = {
          advertAreaIds: location ? [ location ] : [3,4],
          projectId,
