    ...
    <div class="content-main" style="height: calc(100% - 91px);">
      <a-form :form="form" :label-col="{ span: 3 }" :wrapper-col="{ span: 14 }" style="height: 100%;overflow: auto;" autoComplete="off">
        ...
        <a-form-item label="位置">
          <a-select
            :getPopupContainer="(triggerNode)=>{ return triggerNode.parentNode || document.body;}"
            v-decorator="['location',{ initialValue: '3' }]"
            placeholder="请选择位置"
          >
            <a-select-option v-for="(item, key) in locationData" :key="key">{{item}}</a-select-option>
          </a-select>
        </a-form-item>
        ...
        <a-form-item label="投放端">
          <a-radio-group v-decorator="['putPort',{ initialValue: 2 }]" @change="onChange">
            <a-radio :value="2">全部</a-radio>
            <a-radio :value="0">AppC</a-radio>
        ...
        <a-form-item label="设置图片">
          ...
          <span style="margin-left:10px;color:#999999;font-size:12px;">建议上传尺寸为：1016*218，格式为jpg、png，大小不超过5MB。</span>
        </a-form-item>
    ...
    methods: {
      ...
      onCheckAllChange(e) {
        ...
        if (this.checkAll === true) {
          ...
          // console.log()
          // console.log(this.form.getFieldsValue().id)
        } else {
          this.targetKeys = [];
          this.oriTargetKeys = this.targetKeys;
            ...
            if (res.code === 200) {
              ...
              this.$nextTick(()=>{
                this.form.setFieldsValue({
                  ...
                  location: data.advertAreaId.toString(),
                  id: true,
                  putPort: data.launch,
          ...
          this.form.validateFields((err, values) => {
            if (!err) {
              ...
              values = {
                ...
                advertAreaId: Number(values.location),
                advertPicture:this.fileList[0].url,
                projectIds: this.targetKeys,
                launch: Number(values.putPort),
                type: Number(values.type),
                startValidTime: moment(values.date[0]).format("YYYY-MM-DD"),
                endValidTime: moment(values.date[1]).format("YYYY-MM-DD"),
              ...
              api[isAdd ? "addAdvertise" : "updateAdvertise"](values).then(res => {
                if (res.code === 200) {
                  ...
                  this.$router.push({ name: "advertiseArea" });
                }
              }).finally(()=>{
