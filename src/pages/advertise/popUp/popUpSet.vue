    ...
    <div class="content-main" style="height: calc(100% - 91px);">
      <a-form :form="form" :label-col="{ span: 3 }" :wrapper-col="{ span: 14 }" style="height: 100%;overflow: auto;" autoComplete="off">
        ...
        <a-form-item label="设置图片">
          ...
          <span style="margin-left:10px;color:#999999;font-size:12px;">建议上传尺寸为：752*850，格式为jpg、png，大小不超过5MB。</span>
        </a-form-item>
        ...
        <a-form-item label="触发方式">
          <a-select
            :getPopupContainer="(triggerNode)=>{ return triggerNode.parentNode || document.body;}"
            v-decorator="['triggerMode',{ initialValue: '0' }]"
            placeholder="请选择触发方式"
          >
            <a-select-option v-for="(item, key) in triggerModeData" :key="key">{{item}}</a-select-option>
          </a-select>
        </a-form-item>
          ...
          this.form.validateFields((err, values) => {
            if (!err) {
              ...
              values = {
                ...
                triggerMode: Number(values.triggerMode),
