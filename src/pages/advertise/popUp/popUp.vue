<template>
  <div id="popUp">
    <div class="content-header">弹窗管理</div>
...
<script>
  import api from "@/api";
  import { ADVERTISE_SKIP_TYPE,TRIGGER_MODE } from "@/constance";
  ...
  export default {
      ...
      return {
        ...
        tableColumns: [
          ...
          },
          {
            ...
            width: 120,
            ellipsis: true,
            customRender: (val) => TRIGGER_MODE[val] || val
          },
          // {
          //   title: "点击次数",
          //   dataIndex: "name",
    ...
    mounted () {
      this.$refs.form.setFieldsValue({
        ...
        triggerMode: this.formList[4].selectOptions[0]?.id,
      });
    ...
    methods:{
      ...
      onAdd() {
        debounce(()=>{
          this.$router.push({path: "/advertise/popUpSet"})
        })
      },
      //编辑
      onEdit(operation) {
      ...
      getAdvertiseList() {
        ...
        const { projectId, title, type, status, triggerMode, validTime, updateTime   } = this.$refs.form.getFieldsValue();
        const para = {
          advertAreaIds: [2],
          projectId,
