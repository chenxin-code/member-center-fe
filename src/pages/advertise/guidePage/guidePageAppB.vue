...
<script>
  ...
  export default {
    name: "guidePageAppB",
    components: {AFormItem, richTextLike},
    data() {
      return {
    ...
    methods: {
      ...
      getAdvertiseList() {
        ...
        api.getAdvertise(para).then(res => {
          if (res.code === 200) {
            ...
            this.id = res.data.records[0] ? res.data.records[0].id : "";
            this.getData()
          }
        })
      },
      ...
      getData() {
        if (this.id ? this.id : "") {
          api.getAdvertiseDetail({ advertId: this.id }).then(res => {
            if (res.code === 200) {
              const data = res.data;
              this.typeId = res.data.type;
              this.fileList[0] = { uid: "-1", url: res.data.advertPicture };
              this.$nextTick(()=>{
