    ...
    <div class="content-main" style="height: calc(100% - 91px);">
      <a-form :form="form" :label-col="{ span: 3 }" :wrapper-col="{ span: 14 }" style="height: 100%;overflow: auto;" autoComplete="off">
        ...
        <a-form-item label="是否默认">
          <a-select
            :getPopupContainer="(triggerNode)=>{ return triggerNode.parentNode || document.body;}"
            v-decorator="['isDefault',{ initialValue: '1' }]"
            placeholder="请选择是否默认"
            @change="onIsDefault"
          >
            <a-select-option v-for="(item, key) in defaultData" :key="key">{{item}}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="适用项目" v-if="isDefault == 1">
        ...
        <a-form-item label="设置图片">
          ...
          <span style="margin-left:10px;color:#999999;font-size:12px;">建议上传尺寸为：1080*2338，格式为jpg、png，大小不超过5MB。</span>
        </a-form-item>
...
<script>
  ...
  export default {
    ...
    components: {richTextLike,AFormItem,vTransfer },
    props: {
      id: {
        type: Object,
        default: () => ({})
            ...
            if (res.code === 200) {
              ...
              this.$nextTick(()=>{
                this.form.setFieldsValue({
                  ...
                  isDefault: data.isDefault ? data.isDefault.toString() : "",
                  id: true,
            ...
            if (!err) {
              ...
              api[isAdd ? "addAdvertise" : "updateAdvertise"](values).then(res => {
                if (res.code === 200) {
                  ...
                  this.$router.push({ name: "guidePageAppC" });
                }
              }).finally(()=>{
