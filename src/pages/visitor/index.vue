<template>
  <div id="visitor">
    <div class="content-header">访客列表</div>
  ...
  export default {
    ...
    mounted () {
      this.$refs.form.setFieldsValue({
        type: this.formList[3].selectOptions[0]?.id,
        status: this.formList[4].selectOptions[0]?.id,
      });
    ...
    methods:{
      ...
      onAdd() {
        debounce(()=>{
          this.$router.push({path: "/visitor/visitorSet"})
        })
      },
      //查看
      onLook(operation) {
