<template>
  <div class="detail">
    <div class="detail-header">
      ...
      <p class="detail-header-btn" @click="$router.push({path: '/taskCenter-behavior'})">返回</p>
    </div>
    <div class="detail-main">
      <div class="detail-main-base detail-main-items">
        <p class="detail-main-items-title">
        ...
        <div class="detail-main-items-cont">
          <div class="detail-main-items-cont-info">
            <p class="detail-main-items-cont-info-box" v-for="item in baseList" :key="item.label">
              ...
              <span v-if="item.name === 'isUsing'"
                    class="detail-main-items-cont-info-box-value">{{ baseObj[item.name] === 0 ? '禁用' : '启用' }}</span>
              <span v-else-if="item.name === 'type'"
                    class="detail-main-items-cont-info-box-value">{{ baseObj[item.name] === 1 ? '消费' : '其他' }}</span>
              <span v-else-if="item.name === 'createTime'"
                    class="detail-main-items-cont-info-box-value">{{ moment(baseObj[item.name]).format('YYYY-MM-DD HH:mm') }}</span>
...
export default {
  ...
  methods: {
    moment,
    initData(id) {
      api.getDealingDetail({behaviourId: id}).then(res => {
