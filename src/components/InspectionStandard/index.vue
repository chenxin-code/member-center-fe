  ...
  <a-spin :spinning="picUploading">
    ...
    <div>
      <div v-for="(item, index) in list" :key="index" style="display: flex;margin-bottom: 4px;">
        ...
        <div style="width: 20%">
          <a @click="chooseStandard(index)" style="margin-left:38px;">
            {{ item.standardIds.length === 0 ? '选择标准' : '已选' + item.standardIds.length + '个' }}
          </a>
        </div>
        <div style="width: 15%">
          <a-checkbox
            :checked="item.scanFlg == 1 ? true : false"
            :value="item.scanFlg == 1 ? true : false"
            @change="onScanflag($event, index)"
          >
            是否扫码
          </a-checkbox>
        </div>
